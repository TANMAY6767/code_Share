...
const ShareModal = ({ onClose, shareId }) => {
  const [defaultUrl, setDefaultUrl] = useState(`http://localhost:3000/share/${shareId}`);
  const [customUrl, setCustomUrl] = useState('http://localhost:3000/share/');
  ...
  const generateShareLink = () => {
    ...
    const isValid = /^[a-zA-Z0-9-]+$/.test(customUrl);
    if (!isValid) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm">
      <div className="bg-gray-800 rounded-xl shadow-2xl border border-gray-700 w-full max-w-2xl mx-4 animate-[modalSlideIn_0.3s_ease-out]">
